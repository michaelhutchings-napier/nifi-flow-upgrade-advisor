...
FIXTURE_JSON="${ROOT_DIR}/demo/fixtures/jndi-jms-ldap-1.21-flow.json"
RULE_PACK="${ROOT_DIR}/examples/rulepacks/nifi-1.21-to-1.22.official.yaml"
OUT_DIR="${ROOT_DIR}/demo/out/jndi-jms-ldap-1.21-to-1.22"
...
echo "Running JNDI JMS LDAP 1.21.0 -> 1.22.0 manual-change demo"
if run_demo_analyze "${SOURCE_GZ}" "1.21.0" "1.22.0" "${RULE_PACK}" "${OUT_DIR}"; then
  ANALYZE_EXIT=0
else
  ANALYZE_EXIT=$?
fi
...
echo "  - LDAP Provider URL usage is flagged as manual-change"
